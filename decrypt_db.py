...
STATE_FILE = os.path.join(OUT_DIR, ".decrypt_state.json")
...
def decrypt_database(db_path, out_path, enc_key):
    ...
    with open(db_path, "rb") as fin, open(temp_out_path, "wb") as fout:
...
def should_decrypt(rel_path, src_path, out_path, state):
    if not os.path.exists(out_path):
        return True

    current_signature = build_db_signature(src_path)
    cached_signature = state.get(rel_path)
...
def main():
    ...
    os.makedirs(OUT_DIR, exist_ok=True)
    decrypt_state = load_decrypt_state()
    updated_state = dict(decrypt_state)
    ...
    for rel, path, sz in db_files:
        ...
        if not should_decrypt(rel, path, out_path, decrypt_state):
            print(f"SKIP: {rel} (未变化)")
            skipped += 1
            continue
        ...
        if ok:
            ...
            try:
                ...
                success += 1
                total_bytes += sz
                updated_state[rel] = build_db_signature(path)
    ...
    save_decrypt_state(updated_state)
